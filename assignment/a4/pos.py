    ...
    def forward(self, sentence):
        ...
        for i, w in enumerate(sentence):
            for tag in self.tagset:
                ...
                if i == 0:
                    alpha[(i,tag)] = self.initial(tag) + self.emission(tag, w)
                    
                else:
                    list_log_sums = [alpha[(i-1,tag_1)] + self.transition(tag_1, tag) for tag_1 in self.tagset]
                    alpha[(i,tag)] = self.emission(tag, w) + logsumexp(list_log_sums)
